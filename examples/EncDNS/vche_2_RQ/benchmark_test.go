...
var resCt *vche_2.Ciphertext
var ctMask *vche_2.Ciphertext
var tempCt2 *vche_2.Ciphertext
var tempCt1 *vche_2.Ciphertext

var resVerif *vche_2.Poly
var verifMask *vche_2.Poly
var tempVerif2 *vche_2.Poly
var tempVerif1 *vche_2.Poly
...
func init() {
	...
	encoderPlaintext = vche_2.NewEncoderPlaintextRequad(params, sk.K)
	evaluatorPlaintext = vche_2.NewEvaluatorPlaintextRequad(params)
	prover, verifier = vche_2.NewProverVerifier(params, sk, kgen.GenRotationKeysForInnerSum(sk))

	resCt = nil
	ctMask = vche_2.NewCiphertext(params, 1)
	tempCt2 = vche_2.NewCiphertext(params, 2)
	tempCt1 = vche_2.NewCiphertext(params, 1)

	resVerif = nil
	verifMask = vche_2.NewPoly(params)
	tempVerif2 = vche_2.NewPoly(params)
	tempVerif1 = vche_2.NewPoly(params)
}
...
func BenchmarkEncDNS(b *testing.B) {
	...
	resCt, resVerif := benchEval(ctListKeys, ctListVals, encryptedOne, encryptedQuery, indexPlaintext,
		keyTagsVerif, valTagsVerif, oneTagVerif, queryTagsVerif, indexVerif,
		numPacked, b)

	benchVerif(ctListKeys, ctListVals, encryptedOne, encryptedQuery, indexPlaintext,
		keyTagsVerif, valTagsVerif, oneTagVerif, queryTagsVerif, indexVerif,
		numPacked, b)
...
func benchEval(ctListKeys, ctListVals []*vche_2.Ciphertext, encryptedOne, encryptedQuery *vche_2.Ciphertext, maskPlaintext *vche_2.Plaintext,
	verifListKeys, verifListVals []*vche_2.Poly, verifOne, verifQuery *vche_2.Poly, verifMaskPlaintext *vche_2.Poly,
	numPacked int, b *testing.B) (*vche_2.Ciphertext, *vche_2.Poly) {
	...
	b.Run(benchmarkString("Eval"), func(b *testing.B) {
		for run := 0; run < b.N; run++ {
			...
			for i := 0; i < numVec; i++ {
				...
				// Requad
				b.StopTimer()
				evaluatorPlaintext.Mul(verifListKeys[i], verifQuery, tempVerif2)
				evaluatorPlaintext.Relinearize(tempVerif2, verifMask)
				if len(ctMask.Ciphertexts) > 3 {
					numCtxtsSP_C += len(ctMask.Ciphertexts) - 3
					numCtxtsC_SP += 2
				}
				b.StartTimer()
				ctMask, verifMask = vche_2.BenchmarkRequadratizationProtocolProver(prover, verifier, ctMask, verifMask, b)
				b.StartTimer()
				// EndRequad
				...
				evaluatorPlaintext.Sub(verifMask, verifListKeys[i], verifMask)
				evaluatorPlaintext.Sub(verifMask, verifQuery, verifMask)
				evaluatorPlaintext.Add(verifOne, verifMask, verifMask)
				b.StartTimer()
				// EndVerif
				...
				// Verif
				b.StopTimer()
				evaluatorPlaintext.Mul(verifMask, verifMaskPlaintext, verifMask)
				b.StartTimer()
				// EndVerif
				...
				// Requad
				b.StopTimer()
				evaluatorPlaintext.Mul(verifMask, verifListVals[i], tempVerif2)
				evaluatorPlaintext.Relinearize(tempVerif2, tempVerif1)
				if len(tempCt1.Ciphertexts) > 3 {
					numCtxtsSP_C += len(tempCt1.Ciphertexts) - 3
					numCtxtsC_SP += 2
				}
				b.StartTimer()
				tempCt1, tempVerif1 = vche_2.BenchmarkRequadratizationProtocolProver(prover, verifier, tempCt1, tempVerif1, b)
				b.StartTimer()
				// EndRequad
	...
	b.Run(benchmarkString("Communication/Client->SP/Requad"), func(b *testing.B) {
		numCtxts := float64(numCtxtsC_SP) / float64(numRuns)
		b.ReportMetric(numCtxts, "BFV-ctxt")
		// TODO: find a way to log the size in bytes
		b.ReportMetric(0.0, "ns/op")
	})
	b.Run(benchmarkString("Communication/SP->Client/Requad"), func(b *testing.B) {
		numCtxts := float64(numCtxtsSP_C) / float64(numRuns)
		b.ReportMetric(numCtxts, "BFV-ctxt")
		// TODO: find a way to log the size in bytes
		b.ReportMetric(0.0, "ns/op")
	})

	return resCt, resVerif
}

func benchVerif(ctListKeys, ctListVals []*vche_2.Ciphertext, encryptedOne, encryptedQuery *vche_2.Ciphertext, maskPlaintext *vche_2.Plaintext,
	verifListKeys, verifListVals []*vche_2.Poly, verifOne, verifQuery *vche_2.Poly, verifMaskPlaintext *vche_2.Poly,
	numPacked int, b *testing.B) {
	numVec := len(ctListKeys)
	...
	b.Run(benchmarkString("EvalVerif"), func(b *testing.B) {
		for run := 0; run < b.N; run++ {
			...
			for i := 0; i < numVec; i++ {
				...
				ctMask, verifMask = vche_2.BenchmarkRequadratizationProtocolVerifier(prover, verifier, ctMask, verifMask, b)
				...
				evaluatorPlaintext.Mul(verifMask, verifListVals[i], tempVerif2)
				evaluatorPlaintext.Relinearize(tempVerif2, tempVerif1)
				tempCt1, tempVerif1 = vche_2.BenchmarkRequadratizationProtocolVerifier(prover, verifier, tempCt1, tempVerif1, b)
				b.StartTimer()
