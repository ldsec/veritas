...
func init() {
	...
	encoderPlaintext = vche_1.NewEncoderPlaintextCFPRF(params, sk.K)
...
func benchEncVerif(ptTags [][][]vche.Tag, numCt int, b *testing.B) [][]*vche_1.VerifPlaintext {
	cntClient := FedAvg.NumClients
	// Create plaintexts
	polys := make([][]*vche_1.VerifPlaintext, cntClient)
	for i := 0; i < cntClient; i++ {
		polys[i] = make([]*vche_1.VerifPlaintext, numCt)
		for j := 0; j < numCt; j++ {
			polys[i][j] = vche_1.NewVerifPlaintext(params)
		}
	}
...
func benchVerif(ciphertextList [][]*vche_1.VerifPlaintext, numCt int, b *testing.B) []*vche_1.TaggedPoly {
	resCt := make([]*vche_1.VerifPlaintext, numCt)
	res := make([]*vche_1.TaggedPoly, numCt)
