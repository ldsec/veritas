...
func fedAvg(filePath string, cntClient int, params vche_1.Parameters, encoder vche_1.Encoder, encryptorPk vche_1.Encryptor,
	evaluator vche_1.Evaluator, evaluatorPlaintext vche_1.EvaluatorPlaintextCFPRF, evaluatorPlaintextEncoder vche_1.EncoderPlaintextCFPRF, decryptor vche_1.Decryptor) (errcnt int) {
	...
	plaintextListVerif := make([][]*vche_1.VerifPlaintext, cntClient)

	for i := 0; i < cntClient; i++ {
		plaintextList[i] = make([]*vche_1.Plaintext, numCt)
		plaintextListTags[i] = make([][]vche.Tag, numCt)
		plaintextListVerif[i] = make([]*vche_1.VerifPlaintext, numCt)
	...
	// Sum dummies
	resVerif := make([]*vche_1.VerifPlaintext, numCt)
	for j := 0; j < numCt; j++ {
...
func main() {
	fmt.Printf("____ Start BFV FedAvg VCHE1 CFPRF ____ \n")
	...
	encoder := vche_1.NewEncoder(params, sk.K, sk.S, true)
	...
	evaluatorPlaintext := vche_1.NewEvaluatorPlaintextCFPRF(params, sk.H)
	evaluatorPlaintextEncoder := vche_1.NewEncoderPlaintextCFPRF(params, sk.K)
