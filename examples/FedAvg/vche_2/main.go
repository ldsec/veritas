...
func fedAvg(filePath string, cntClient int, params vche_2.Parameters, encoder vche_2.Encoder, encryptorPk vche_2.Encryptor,
	evaluator vche_2.Evaluator, evaluatorPlaintext vche_2.EvaluatorPlaintext, evaluatorPlaintextEncoder vche_2.EncoderPlaintext, decryptor vche_2.Decryptor) (errcnt int) {
	...
	plaintextListVerif := make([][]*vche_2.Poly, cntClient)

	for i := 0; i < cntClient; i++ {
		plaintextList[i] = make([]*vche_2.Plaintext, numCt)
		plaintextListTags[i] = make([][]vche.Tag, numCt)
		plaintextListVerif[i] = make([]*vche_2.Poly, numCt)
	...
	// Sum dummies
	resVerif := make([]*vche_2.Poly, numCt)
	for j := 0; j < numCt; j++ {
	...
	fmt.Printf("soundness PE2: %v\n", math.Log2( float64(resCt[0].Len()-1)*(float64(1)/float64(params.T()) + float64(1)/float64(params.T()-1) ) ) )
    fmt.Printf("soundness  PEPP: %v\n", math.Log2( 2*float64(resCt[0].Len()-1+params.NSlots)/float64(params.T()) + float64((resCt[0].Len()-1))/float64(params.T()-1) )  )
    fmt.Printf("d: %v\n", (resCt[0].Len()-1))
...
func main() {
	fmt.Printf("____ Start BFV FedAvg VCHE2 ____ \n")
