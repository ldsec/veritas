...
func benchEncVerif(ptTags [][][]vche.Tag, numCt int, b *testing.B) [][]*vche_2.Poly {
	cntClient := FedAvg.NumClients
	// Create plaintexts
	polys := make([][]*vche_2.Poly, cntClient)
	for i := 0; i < cntClient; i++ {
		polys[i] = make([]*vche_2.Poly, numCt)
		for j := 0; j < numCt; j++ {
			polys[i][j] = vche_2.NewPoly(params)
		}
	}
...
func benchVerif(ciphertextList [][]*vche_2.Poly, numCt int, b *testing.B) []*vche_2.Poly {
	resCt := make([]*vche_2.Poly, numCt)
