...
func BenchmarkVCHE2(b *testing.B) {
	var err interface{}
...
func benchEvaluator(testctx *testContext, b *testing.B) {
	...
	evaluator := testctx.evaluator.WithKey(EvaluationKey{Rlk: testctx.rlk, Rtks: rotkey})
...
func benchEvaluatorPlaintext(testctx *testContext, b *testing.B) {
	...
	receiver := NewPoly(testctx.params)

	evaluator := testctx.evaluatorPlaintext

	b.Run(testStringNoSplit("EvaluatorPlaintext/Add/Ct/", testctx.params), func(b *testing.B) {
		for i := 0; i < b.N; i++ {
