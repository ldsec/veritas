...
func TestRunRequadratizationProtocol(t *testing.T) {
	for _, paramDef := range DefaultParams[1:] { // Skip PN12, since we want multiplicative depth >= 2 for the tests
		...
		t.Run(testString("ReQuad(c4)/", params), func(t *testing.T) {
			ctxtRes, verifRes := RunRequadratizationProtocol(testctx.prover, testctx.verifier, ctxt4, verif4)
			valuesRes := values4
		...
		t.Run(testString("ReQuad(c4)+ReQuad(c4+c1)/", params), func(t *testing.T) {
			ctxt2, verif2 := RunRequadratizationProtocol(testctx.prover, testctx.verifier, ctxt4, verif4)

			ctxt4_ := testctx.evaluator.AddNew(ctxt4, ctxt1)
			verif4_ := testctx.evaluatorPlaintext.AddNew(verif4, verif1)
			ctxt2_, verif2_ := RunRequadratizationProtocol(testctx.prover, testctx.verifier, ctxt4_, verif4_)
		...
		t.Run(testString("ReQuad(c4_1)/", params), func(t *testing.T) {
			ctxtRes, verifRes := RunRequadratizationProtocol(testctx.prover, testctx.verifier, ctxt4_1, verif4)
			valuesRes := values4
			verifyTestVectors(testctx, testctx.decryptor, valuesRes.Coeffs[0], ctxtRes, verifRes, false, t)
		})

		t.Run(testString("ReLin(ReQuad(c4))/", params), func(t *testing.T) {
			evk := EvaluationKey{Rlk: testctx.kgen.GenRelinearizationKey(testctx.sk, 6), Rtks: nil}
		...
		t.Run(testString("RotateRows(ReQuad(c4_1))/", params), func(t *testing.T) {
			evk := EvaluationKey{Rlk: nil, Rtks: testctx.kgen.GenRotationKeysForRotations([]int{0}, true, testctx.sk)}
			eval := testctx.evaluator.WithKey(evk)
		...
		t.Run(testString("InnerSum(ReQuad(c4))/", params), func(t *testing.T) {
			...
			sum := big.NewInt(0)
			bigT := big.NewInt(0).SetUint64(testctx.params.T())
			for _, c := range vals {
				sum.Add(sum, big.NewInt(0).SetUint64(c))
				sum.Mod(sum, bigT)
			}
		...
		t.Run(testString("ReQuad(ReQuad(c4)*p1)/", params), func(t *testing.T) {
			...
			ctxtRes, verifRes = RunRequadratizationProtocol(testctx.prover, testctx.verifier, ctxtRes, verifRes)
